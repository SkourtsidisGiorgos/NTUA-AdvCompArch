...
def get_time_from_sim_file(time_file):
    time=float("NaN")
    try:
        fp = open(time_file, "r")
    except IOError:
...
for dirname in sys.argv[1:]:
    ...
    time_file   = dirname + "/sim.out"

    (bench, input_size, thr, gs) = get_params_from_basename(basename)


    joule = get_ipc_from_output_file(output_file)
    time  = get_time_from_sim_file(time_file) / 1000000000  # convert to sec from ns
    edp = joule * time
    results_tuples.append((bench, thr, edp))
...
plt.savefig("EDP_GS="+str(gs)+'.ipc.png', bbox_extra_artists=(lgd,), bbox_inches='tight')
