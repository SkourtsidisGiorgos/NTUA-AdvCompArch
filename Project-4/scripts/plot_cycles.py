...
def get_ipc_from_output_file(output_file,thr):
	ipc = float("NaN")
	try:
...
for dirname in sys.argv[1:]:
	...
	ipc = get_ipc_from_output_file(output_file,thr)
...
lgd = ax.legend(locks_names,ncol=len(tuples_by_dw), bbox_to_anchor=(1, -0.12), prop={'size':8})
plt.savefig("CYCLES_GS="+str(gs)+'.ipc.png', bbox_extra_artists=(lgd,), bbox_inches='tight')
